...
public class FirstNameUtility {
    ...
    public int encodeFirstName(String firstName, String middleInitial) throws MissingNameException {
        if (firstName.isBlank()) {
            ...
            int firstNameValue;
            int middleInitialValue;

            if (firstNameCodeMap.containsKey(firstName.toUpperCase())) {
                firstNameValue = firstNameCodeMap.get(firstName.toUpperCase());
            } else {
                String firstInitial = firstName.substring(0, 1);
                firstNameValue = firstInitialCodeMap.get(firstInitial.toUpperCase());
            }

            middleInitialValue = middleInitial.isBlank()
                    ? 0
                    : middleInitialCodeMap.get(middleInitial.substring(0, 1).toUpperCase());

            return firstNameValue + middleInitialValue;
        }
    }
}
